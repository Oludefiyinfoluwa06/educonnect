...
const PaymentCard: React.FC<Payment> = ({ className, studentName, amount, paymentDate, status }) => {
    const statusColors = {
    ...
    return (
        <View className="bg-white p-4 rounded-xl shadow-sm mb-3">
            <View className="flex-row justify-between items-center mb-2">
                <Text className="text-gray-800 font-rsemibold text-lg">{className} - {studentName}</Text>
        ...
        <SafeAreaView className="flex-1 bg-gray-50">
            <ScrollView className="flex-1 px-4">
                <View className="flex-row justify-between items-center py-4">
                    <Text className="text-xl font-rbold text-gray-800">Payments</Text>
                    <TouchableOpacity
                        className="bg-blue-600 px-4 py-2 rounded-full"
                        onPress={() => setNewPayment({ className: 'JSS 1', studentName: '', amount: 0, paymentDate: '' })}
                    >
                        <Text className="text-white font-rmedium">Add Payment</Text>
                    </TouchableOpacity>
                </View>
                ...
                {newPayment.amount > 0 && (
                    <View className="bg-white p-4 rounded-xl shadow-sm mb-4">
                        ...
                        <Picker
                            className="mb-3"
                            selectedValue={newPayment.className}
                            onValueChange={(itemValue: 'JSS 1' | 'JSS 2' | 'JSS 3' | 'SSS 1' | 'SSS 2' | 'SSS 3') =>
                                setNewPayment({
                                    ...newPayment,
                                    className: itemValue as 'JSS 1' | 'JSS 2' | 'JSS 3' | 'SSS 1' | 'SSS 2' | 'SSS 3',
                                })
                            }
                        >
